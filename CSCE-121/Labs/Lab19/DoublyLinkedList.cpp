...
bool operator==(DoublyLinkedList const& lhs,DoublyLinkedList const& rhs)
{	const Node* Hlhs = &lhs.get_head();
	const Node* Hrhs = &rhs.get_head();
    while (Hrhs) { 
    	if (Hlhs->next == nullptr && Hrhs->next != nullptr) 
    		return false; 
    	if (Hrhs->next == nullptr && Hlhs->next != nullptr) 
    		return false; 
    	if (Hrhs->data != Hlhs-> data) 
    		return false; 
    	Hlhs = Hlhs->next; 
    	Hrhs = Hrhs->next; 
    }
...
DoublyLinkedList& DoublyLinkedList::operator +=(DoublyLinkedList const& rhs)
{
	const Node* r = &rhs.get_head();
	if(r == nullptr)
		return *this;
	while(r->next)
	{
